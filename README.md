...
- **Deployment**: not yet deployed. The spec calls for cloud hosting (Vercel + managed Postgres). This build currently targets local Postgres; before deploying, point `DATABASE_URL` at a managed Postgres instance (Neon, Supabase, Railway, etc.) and re-run migrations against it.
